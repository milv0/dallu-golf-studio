...
import { hrefLang, resolveInitialLang } from "./appLanguage.js";
...
export function LangProvider({ children, lang: routeLang }) {
  ...
  // 한국어 페이지로 튕긴다. 앱은 /en 트리로 건너가면 루트 레이아웃 교체로 전체 문서
  // 로드가 일어나 화면이 멈추므로 항상 한국어 트리에 머문다 (언어는 상태가 담당).
  const href = useCallback(
    (path) => withLang(path, hrefLang({ lang, nativeApp })),
    [lang, nativeApp]
  );
