...
- 홈의 정보 계층은 `상단 앱바 → 스코어카드 형식 선택 → 내 라운드 기록 → 설치 버튼` 순서다.
- 직접 만들기의 18/9/3/1홀은 같은 크기의 2×2 정사각형 카드로 바로 진입한다. 별도 `직접 만들기` 허브를 거치지 않는다.
- `내 라운드 기록`: 라운드 입력 기반 연동 작업. 현재는 카드 아래 한 줄의 비활성 항목으로 표시한다.
...
- 설치 안내는 MyRound 아래의 48px 버튼으로 둔다. 절차는 버튼을 눌렀을 때 네이티브 다이얼로그 바텀 시트로 연다.
...
- `AppHeader`: 홈·가이드·작업 화면이 공유하는 첫 행. 브랜드 + Q&A/테마/언어/로그인.
- `StudioNav`: `AppHeader` 아래의 작업 탭을 렌더링. 활성 탭은 크기/두께로 강조.
